...
(double srr, double srt, double str, double stt){
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){
    ...
    /**retireve the position from the reading, and compute the odometry 从机器人的位置读取里程计信息*/
    OrientedPoint relPose=reading.getPose();
    if (!m_count){
      m_lastPartPose=m_odoPose=relPose;  //relPoses是当前时刻位姿 m_odoPose是上一时刻的位姿
    }
    
    //write the state of the reading and update all the particles using the motion model根据运动模型更新所有粒子的状态
    for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
      OrientedPoint& pose(it->pose);   //声明一个Pose用于存储从运动模型中采样出来的位姿
      pose=m_motionModel.drawFromMotion(it->pose, relPose, m_odoPose); //第一个参数是输出，第二个参数是当前位姿，m_odoPose是上一时刻的位姿
	...
	plainReading[i]=reading[i];
      ...
      RangeReading* reading_copy = 
              new RangeReading(reading.size(),
                               ...
                               reading.getTime());
//在从运动模型采样后  也就是预测了粒子的位姿之后，接下来就是要根据测量数据对当前的粒子的位姿进行更新，
//首先是scanMatch()因为根据测量与之前的地图的比较能够更新粒子的最优位子。
/*在程序中的顺序是scanMatch()此函数的具体是：GridSlamProcessor::scanMatch()实现过程是在此函数的具体是：GridSlamProcessor.hxx文件中
    接下来就是在这里scanMatch()函数中的函数 ：
	（1）计算最优粒子的修正后的位姿并返回其得分score=m_matcher.optimize(corrected, it->map, it->pose, plainReading); 
	这里的corrected就是根据测量过得的修正后的粒子位姿             那么在这个optimizze()函数中用到了
	double ScanMatcher::optimize(OrientedPoint& pnew, const ScanMatcherMap& map, const OrientedPoint& init, const double* readings)
   这个函数主要是为了调整粒子以获得最优位姿，其中又用到了double currentScore=score(map, currentPose, readings);计算粒子得分的方法。
   以上就是一个optimize()函数所涉及到最重要的函数
   （2）现在我们返回到scanMatch（）函数中 之后就是要根据当前粒子计算粒子的权重：
   m_matcher.likelihoodAndScore(s, l, it->map, it->pose, plainReading);
   （3）根据粒子的权重，更新地图
   m_matcher.computeActiveArea(it->map, it->pose, plainReading); //用于计算每个粒子相应的位姿所扫描到的区域 
那么这只一个scanMatch()函数的作用。
*/
      if (m_count>0){
...
至此，processScan 结束，回到laserCallback，还有最优一步updateMap
	*/
 	resample(plainReading, adaptParticles, reading_copy); //重采样
    ...
    m_readingCount++;
    return processed;     //返回一个是否对新来的数据匹配成功的bool型变量。
  }
