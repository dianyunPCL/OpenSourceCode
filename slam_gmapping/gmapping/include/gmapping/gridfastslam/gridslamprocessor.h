...
namespace GMapping {
  ...
  /**This class defines the basic GridFastSLAM algorithm.  It
	 ...
	 中文：这个类定义了基本的栅格SLAM 的方法的算法，实现了RBPF滤波算法，每一个粒子都是
	 都拥有自己的局部地图，和位姿，具体的实现的工作如下所示：
	 每一个时刻都有一对里程计信息和激光雷达的数据获取，，那么粒子机器人的位姿是根据
	 运动模型更新的。这个粒子位姿随后就用来初始化激光数据的匹配。，当然激光的匹配表现出的
	 是每个粒子的局部最优的效果，，
	 这是一个从运动模型中获取粒子位姿，之后根据每个粒子的地图更正机器人的位姿，
	 
	 为了更好的避免不必要的计算，滤波器的状态是要根据机器人的移动超过一定的阀值才会更新。
  */
  ...
  public:
    ...
    /**This class defines the the node of reversed tree in which the trajectories are stored.
	   ...
	   中文个人理解：这个类定义了存储轨迹的反向树节点，树的每个节点都有一个指向其父节点的指针和一个指示节点的子节点数量。
       树与粒子上执行的操作相一致的方式更新。
   */
    struct TNode{
      /**Constructs a node of the trajectory tree.  构造一个节点的轨迹树
       @param pose:      the pose of the robot in the trajectory   在轨迹中机器人的位姿
       @param weight:    the weight of the particle at that point in the trajectory  轨迹中在当前时刻粒子的权重
       @param accWeight: the cumulative weight of the particle  粒子的累计权重
       @param parent:    the parent node in the tree  数的父节点
       @param childs:    the number of childs   节点的子数目
      */
  ...
  //这个类主要用于定义粒子滤波的信息，每个粒子都有一副局部地图，一个位姿 一个权重 在轨迹树中保留当前节点
	struct Particle{
      /**constructs a particle, given a map
	 @param map: the particle map   
      */
      Particle(const ScanMatcherMap& map);   //一个粒子拥有一个map

      /** @returns the weight of a particle 返回粒子的权重*/
      inline operator double() const {return weight;}  
	  
      /** @returns the pose of a particle 粒子的位姿*/
      inline operator OrientedPoint() const {return pose;}
      /** sets the weight of a particle  设置粒子的权重
	  @param w the weight
      */
      inline void setWeight(double w) {weight=w;}
      /** The map */
      ScanMatcherMap map;
      /** The pose of the robot 机器人的位姿*/
      OrientedPoint pose;

      /** The pose of the robot at the previous time frame (used for computing thr odometry displacements)之前机器人的位姿 */
    ...
    typedef std::vector<Particle> ParticleVector;   //为粒子申明一个容器  因为会有很多的粒子啊
    
    /** Constructs a GridSlamProcessor, initialized with the default parameters 初始化为默认的参数*/
    GridSlamProcessor();
    ...
    */
    ...
    /**the scanmatcher algorithm   scan -matcher算法*/
