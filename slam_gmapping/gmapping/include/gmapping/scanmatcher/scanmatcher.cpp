...
void ScanMatcher::invalidateActiveArea(){
	m_activeAreaComputed=false;  ////state of the matcher
}
...
void ScanMatcher::computeActiveArea(ScanMatcherMap& map, const OrientedPoint& p, const double* readings){
	...
	Point min(map.map2world(0,0));  //转换到x-y地图空间的最小距离
	Point max(map.map2world(map.getMapSizeX()-1,map.getMapSizeY()-1));//转换后的最大的xy的距离
	...
	/*determine the size of the area 决定区域的大小 */    
	const double * angle=m_laserAngles+m_initialBeamsSkip;   //角度
	for (const double* r=readings+m_initialBeamsSkip; r<readings+m_laserBeams; r++, angle++){
		if (*r>m_laserMaxRange||*r==0.0||isnan(*r)) continue;//如果有大于激光数据设定的最大的距离以及无效的点就省去继续
		double d=*r>m_usableRange?m_usableRange:*r;
		Point phit=lp;
		phit.x+=d*cos(lp.theta+*angle);    //计算phit的也就是障碍物的距离
	...
	for (const double* r=readings+m_initialBeamsSkip; r<readings+m_laserBeams; r++, angle++)
		if (m_generateMap){
			...
			GridLineTraversal::gridLine(p0, p1, &line);  //关于地图的显示以及表示的相关
			for (int i=0; i<line.num_points-1; i++){
...
//registerScan()与之前的GridSlamProcessor::scanMatch()是不一样的这个输入的是地图，位姿以及新加入的数据
double ScanMatcher::registerScan(ScanMatcherMap& map, const OrientedPoint& p, const double* readings){
	if (!m_activeAreaComputed)
		computeActiveArea(map, p, readings);   //首先对新的数据的最大值范围计算是否需要在现有的地图上扩张
		
	//this operation replicates the cells that will be changed in the registration operation
	map.storage().allocActiveArea();
	
	OrientedPoint lp=p;  //计算x,y,theta的数据的增量
	lp.x+=cos(p.theta)*m_laserPose.x-sin(p.theta)*m_laserPose.y;
	lp.y+=sin(p.theta)*m_laserPose.x+cos(p.theta)*m_laserPose.y;
	lp.theta+=m_laserPose.theta;
	IntPoint p0=map.world2map(lp);  //转换到地图中
...
double ScanMatcher::optimize(OrientedPoint& pnew, const ScanMatcherMap& map, const OrientedPoint& init, const double* readings) const{
	...
	//score（）函数也就是寻找最优粒子的函数。
	double currentScore=score(map, currentPose, readings);// 返回当前位姿的最优的粒子也就是传说中的根据测量数据更新的粒子的位姿并计算得分
	double adelta=m_optAngularDelta, ldelta=m_optLinearDelta;  //最优角度差值   最优线性差值
	unsigned int refinement=0;
	enum Move{Front, Back, Left, Right, TurnLeft, TurnRight, Done};//调整粒子的六个位置
...
*/	int c_iterations=0;  //迭代次数
	do{
		if (bestScore>=currentScore){ //如果定义的BestSocre大于之前找到的最优粒子的分数
			refinement++;
			adelta*=.5;  //那么就需要重新定义角度和线性距离的差值  这个差值就就是粒子周围的距离，就是判断附近点的距离
			ldelta*=.5;
		}
		bestScore=currentScore;  //知道两者相同
...
//		cout <<  "pose=" << currentPose.x  << " " << currentPose.y << " " << currentPose.theta << endl;
		...
		Move move=Front;  //对寻找最优粒子进行微调  以寻找最优的粒子 并返回得分
		do {
			...
			if (m_linearOdometryReliability>0.){
				...
				odo_gain*=exp(-m_linearOdometryReliability*drho);//修正粒子得分的增益
			}
			double localScore=odo_gain*score(map, localPose, readings);  //将计算出来的粒子的得分增益直接与之前的值相乘
...
//		cout << "currentScore=" << currentScore<< endl;
	...
	pnew=currentPose;  //就是通过测量获得的修正后的粒子的位姿。
	return bestScore;  //返回修正后粒子的得分。
}
