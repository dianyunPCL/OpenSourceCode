...
main (int argc, char ** argv)
{
  ...
  float radius = -1;
  if (pcl::console::find_switch (argc, argv, "-radius")) //半径搜索
    pcl::console::parse (argc, argv, "-radius", radius);

  int k = -1;
  if (pcl::console::find_switch (argc, argv, "-knn"))  //K临域搜索
  ...
  pcl::search::KdTree<pcl::PointXYZRGB> tree;   //KD树搜索方式
  pcl::PointXYZRGB query;  //搜索的点为（0.5,0.5,0.5）
  query.x = 0.5;
  query.y = 0.5;
  query.z = 0.5;
